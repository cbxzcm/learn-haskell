module Chapter11.PhoneExercises where
...
findButtonsMaybe buttons = fmap (findButtonMaybe buttons)

findButtonMaybe :: [Button] -> Value -> Maybe (Digit, Presses)
findButtonMaybe buttons target = do
  button <- find (elem target . values) buttons
...
compareGroups (_, aCount) (_, bCount)
  | aCount < bCount = LT
  | aCount > bCount = GT
  | otherwise = EQ
