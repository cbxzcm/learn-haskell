...
isSubseqOf [] _ = True
isSubseqOf _ [] = False
isSubseqOf subSequence@(x : xs) (y : ys)
...
capitalizeWords input = fmap capitalizeWord (words input)
  where
    capitalizeWord originalWord@(x : xs) = (originalWord, toUpper x : xs)

capitalizeWordsTest :: String -> [(String, String)] -> IO ()
capitalizeWordsTest input expected =
  if capitalizeWords input == expected
    then
      putStrLn
        ( "[PASS] input = \""
            ++ input
...
capitalizeWordTest input expected =
  if capitalizeWord input == expected
    then
      putStrLn
...
capitalizeParagraph [] = []
capitalizeParagraph x = intercalate ". " (capitalizeSentences x) ++ "."
  where
